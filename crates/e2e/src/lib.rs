...
impl Builder {
    ...
    /// Execute `test`.
    ///
    /// Spawns the `tvc_gpg` binary, waits for it to bind, then runs
    /// the provided test function with a [`TestArgs`] containing the base URL.
    ///
    /// Note this test env builder relies on the `tvc_gpg` binary already
        ...
        T: Future<Output = ()>,
    {
        ...
        let server_binary = assert_cmd::cargo::cargo_bin("tvc_gpg");
