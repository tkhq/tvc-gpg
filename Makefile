HOST ?= 127.0.0.1
PORT ?= 44020
LOCAL_ENCLAVE_DIR ?= /tmp/tvc-gpg-local-enclave
...
.PHONY: run
run: local-keys
	cargo run --bin tvc_gpg -- \
	--host $(HOST) \
	--port $(PORT) \
	--quorum-file $(QUORUM_FILE) \
	--app-id local

out/tvc-gpg/index.json: \
	Cargo.lock Cargo.toml rust-toolchain.toml $(shell find images/tvc-gpg crates -type f ! -path '*/target/*')
	$(call build,tvc-gpg)
...
define build
	...
	docker build \
		...
		--progress=plain \
		--platform=$(PLATFORM) \
		--label "org.opencontainers.image.source=https://github.com/tkhq/tvc-gpg" \
